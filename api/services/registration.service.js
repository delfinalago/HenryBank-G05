    ...
    actions: {
        ...
        user: {
            ...
            async handler(ctx) {
				...
				const valDni1 = await this.validateDni1(dni);
                if (!valDni1) {
                    return {
                        error:
                            "el DNI existe",
    ...
    methods: {
        ...
        async validateDni(dni) {
			...
			validateDni1(dni) {
				...
				return axios
					.get(
						`https://anypoint.mulesoft.com/mocking/api/v1/sources/exchange/assets/083c0681-d272-40f7-b1ff-90dd5952a0de/personas/1.0.1/m=${dni}`
					)
					.then(({data}) => {
						if (data.dniApi.length) {
							return data.dniApi[0].confirmDni;
						} else {
							return false;
						}
					});
